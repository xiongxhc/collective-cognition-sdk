...
- [RFC 0003: Portable Cognition Contract](0003-portable-cognition-contract.md) — implemented and final-review verified in Phase 3 with a Normative Stable `0.1.0` exchange envelope and a historical private package `0.2.0` surface; the current package is private, unpublished `0.10.0`.
...
- [RFC 0011: Cross-Connector Interoperability](0011-cross-connector-interoperability.md) — implemented and locally verified for private, unpublished package `0.10.0`, with two maintained connectors, Interoperability Profile `0.1.0`, and a reference exchange owned by `collective-cognition-sdk-maintainers`; merge and post-merge CI remain pending. The Git connector performs read-only first-parent collection from an explicit local repository and exact tip through a local Git executable with privacy defaults off. There is no Git CLI, connector registry, plugin discovery or runtime, network access, scheduling, or automatic cognition. The profile is not production certification, does not imply endorsement, and is not an LTS commitment.
