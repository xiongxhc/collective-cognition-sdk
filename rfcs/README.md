...
- [RFC 0003: Portable Cognition Contract](0003-portable-cognition-contract.md) — implemented and final-review verified in Phase 3 with a Normative Stable `0.1.0` exchange envelope and a historical private package `0.2.0` surface; the current package is private, unpublished `0.8.0`.
...
- [RFC 0008: Runtime and Security Profile](0008-runtime-security-profile.md) — implemented and verified with a versioned JSON inventory at `collective-cognition-sdk/runtime-security/0.1.0`, explicit `sdk-enforced`, `conformance-verified`, `host-required`, and `out-of-scope` classes, and a boundary that leaves authentication, encryption, tenant or workspace isolation, and durable publication recovery host-owned. At that slice boundary, the current package is private, unpublished `0.7.0`; current package `0.8.0` supersedes it additively. Conformance is not certification.
- [RFC 0009: Public API and Distribution Readiness](0009-public-api-and-distribution-readiness.md) — implemented for current private, unpublished package `0.8.0` with a checked [public API reference](../docs/public-api.md), Normative Stable Distribution Readiness Profile `0.1.0` [prose](../spec/distribution-readiness.md) and [JSON](../spec/distribution-readiness/0.1.0/profile.json), and additive compatibility evidence. It reports npm as blocked and production use as not claimed; reading the profile grants no publication authority.
