...
- `<event-key>` is the lowercase hexadecimal SHA-256 digest of the UTF-8 event
  ID; and
- object versions are zero-padded positive integers from `1` through
  `99,999,999`.

The same `99,999,999` ceiling applies to cognition-event `objectVersion`.
`MARKDOWN_COGNITION_MAX_OBJECT_VERSION` exports this adapter-specific limit.
All profile and projection entrypoints reject larger otherwise-valid Portable
Cognition records with `projection_limit_exceeded` before target inspection or
other filesystem I/O.
...
- input bytes;
- record count;
- object revisions and cognition-event target versions;
