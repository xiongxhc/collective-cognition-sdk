...
| `HIC-014` | Invalid host requests MUST fail before invoking an adapter. For coordinator-driven initial or transition commits and publication attempts, coordinator-generated `HostFailure` values for adapter exceptions, malformed commit results, and invalid publication statuses MUST use the stable sanitized commit or publication failure outcome and MUST NOT expose adapter exception text, secrets, paths, or operational details. Conformance reports MUST use their fixed sanitized failure message. This rule does not standardize thrown or rejected raw read-port operations. |
...
| `HIC-002` | Runtime and focused test: typed public ports accept Portable Cognition record families; the port-isolation test in `tests/host-conformance.test.ts` verifies only cognitive-object and cognition-event records reach ports. |
...
| `HIC-007` | Conformance and prose: `HIC-CONF-007` proves object-and-event read-back after success and no partial visibility after a returned event-ID conflict; ambiguous adapter failure recovery is specified by the identical-request retry requirement. |
...
| `HIC-009` | Runtime and conformance: canonical reference-store checks plus `HIC-CONF-002`, `HIC-CONF-003`, `HIC-CONF-005`, `HIC-CONF-007`, and `HIC-CONF-012` cover replay and immutable identities. |
...
| `HIC-014` | Runtime and conformance: `tests/host-integration.test.ts` covers coordinator-generated sanitized commit and publication failures, while the final conformance test covers the runner's fixed sanitized failure message; raw read-port errors have no standardized `HostFailure` mapping. |
