...
author-email fields behind disabled-by-default privacy options. The
[Git connector guide](../docs/git-connector-guide.md) defines its closed
options and sanitized errors. There is no Git CLI.

[Cross-Connector Interoperability Profile `0.1.0`](interoperability.md) and its
versioned resources are owned by `collective-cognition-sdk-maintainers`. The
profile proves the maintained pair can use one generic ingestion path; it adds
no connector registry, plugin discovery or runtime, network connector,
scheduler, or automatic cognition.
