...
- Records [read-only durable-workflow acceptance](docs/acceptance/durable-cognition-workflow-0.1.0.md) over `12` bounded canonical SourceRecords: exact commit/replay and Markdown statuses, `3` object rows, `1` event, `1` workflow receipt, neutral Evidence with `12` provenance records, `0` Decisions, `0` Principles, equal before/after source identity and SHA-256 values, and no live-vault access. Implementation and acceptance are complete; Task 8 final review remains pending.
